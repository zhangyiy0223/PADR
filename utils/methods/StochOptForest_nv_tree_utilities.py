...
def compute_quantile_general_level(Y, levels):
    ...
    sol_temp = np.zeros(levels.shape[0])
...
def compare_forest_one_run(X, Y, X_est, Y_est, 
    ...
    if oracle:
        ...
        forest_temp = forest(
            ...
            hessian_computer = partial(compute_hessian, h_list = h_list, b_list = b_list, C = C), 
            gradient_computer = partial(compute_gradient, h_list = h_list, b_list = b_list, C = C), 
            search_active_constraint = partial(search_active_constraint, C = C),
            compute_update_step = partial(compute_update_step, constraint = True),
            crit_computer = partial(compute_crit_oracle, solver = opt_solver), 
            impurity_computer = partial(impurity_oracle, h_list = h_list, b_list = b_list, C = C), 
            
            subsample_ratio=subsample_ratio, # 1
            bootstrap = bootstrap, # True
            n_trees = n_trees, # 500
            honesty = honesty, # False
            mtry = mtry, # p=10
            min_leaf_size = min_leaf_size, # 10
            max_depth = max_depth, # 100
            n_proposals = n_proposals, # 100, 200, 400, 800
            balancedness_tol = balancedness_tol, # 0.2
