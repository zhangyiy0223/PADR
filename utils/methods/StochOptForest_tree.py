# Source: https://github.com/CausalML/StochOptForest
...
class forest:
    ...
    def fit_forest(self, Y, X, Y_est, X_est):
        ...
        trees = []
        for s in subsample_ind: # subsample indices for each tree
            # try:
            if not self.honesty:
                Y_train = Y[s]; X_train = X[s]
                tree = build_tree(Y_train, X_train, Y_est, X_est, opt_solver = self.opt_solver,
                                  compute_update_step = self.compute_update_step,
                                  ...
                                  n_proposals = self.n_proposals, balancedness_tol = self.balancedness_tol, verbose = self.verbose, seed = self.seed) # todo
            if self.honesty:
